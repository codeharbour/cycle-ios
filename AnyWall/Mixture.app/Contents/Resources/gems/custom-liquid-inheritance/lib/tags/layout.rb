  ...
  class Layout < ::Liquid::Block
    ...
    def initialize(tag_name, markup, tokens)     
      ...
      if markup =~ Syntax 
        ...
        raise Liquid::SyntaxError.new("Syntax Error in 'layout' - Valid syntax: layout [template]")
      end
    ...
    def load_template(context)
      ...
      layouttemp = "layouts/"+context[@template_name]
      
      source = Liquid::Template.file_system.read_template_file(layouttemp,context)      
    ...
    def is_layout?(template)
      template.root.nodelist.any? { |node| node.is_a?(Layout) }
    end
