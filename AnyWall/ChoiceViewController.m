...
- (void)viewDidLayoutSubviews {
	...
	tableViewFrame.origin.y = 150.0f;//CGRectGetMaxY(self.mapView.frame) + 6.0f;
	tableViewFrame.size.width = CGRectGetMaxX(bounds) - CGRectGetMinX(tableViewFrame) * 2.0f;
	tableViewFrame.size.height = CGRectGetMaxY(bounds) - CGRectGetMaxY(tableViewFrame) - 50 - 150;
...
- (void)queryForAllPostsNearLocation:(CLLocation *)currentLocation withNearbyDistance:(CLLocationAccuracy)nearbyDistance {
	...
	[query whereKey:PAWParsePostLocationKey nearGeoPoint:point withinKilometers:0.1];
